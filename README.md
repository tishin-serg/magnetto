...
- минимальная скорость в МБ/с (`0` отключает наблюдение);
- автоматическая замена медленной раздачи.
...
### Контроль скорости в начале загрузки

Порог скорости и режим автозамены копируются из настроек в новую заявку. После
перехода torrent в `DOWNLOADING` бот раз в минуту проверяет скорость в течение
15 минут. Алерт срабатывает один раз после трёх последовательных низких замеров;
ошибка связи с qBittorrent не считается нулевой скоростью.

Без автозамены текущая загрузка продолжается, пока пользователь просматривает
сохранённые альтернативы. После подтверждения старый torrent ставится на паузу
без удаления данных, а выбранная альтернатива запускается как новая заявка с той
же целью. При включённой автозамене первый сохранённый вариант выбирается
автоматически. Для ручной magnet-ссылки без альтернатив предлагается новый поиск.
...
Перед запуском загрузки в VPS- или домашнюю медиатеку бот запрашивает свободное
место у выбранного qBittorrent. Если известный заранее или полученный из metadata
размер выбранных файлов превышает доступное место, torrent не запускается и бот
показывает ошибку. Для S3 эта проверка не выполняется.
...
| MGB-11 | Проверка свободного места выбранного VPS/Home qBittorrent до начала загрузки; S3 исключён. Проверка: 155 тестов, `mvnw verify`. |
...
При включённом API мобильный клиент доступен по `/api/iphone`. Токен сохраняется
локально в браузере iPhone и передаётся только в заголовке авторизации. Параметры
`q` и `target` в URL позволяют Siri Shortcut сразу открыть результаты поиска для
выбранного назначения, не помещая токен в ссылку.
